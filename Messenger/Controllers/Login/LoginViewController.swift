...
class LoginViewController: UIViewController {
    ...
    @objc private func logginButtonTapped () {
        ...
        guard let email = emailField.text, let password = passwordField.text,
              !email.isEmpty, !password.isEmpty, password.count > 3 else {
            alertUserLoginError()
            return
        }
    ...
    func alertUserLoginError () {
        let alert = UIAlertController(title: "warning", message: "please nter all field", preferredStyle: .alert)
    ...
    func loginButton(_ loginButton: FBLoginButton, didCompleteWith result: LoginManagerLoginResult?, error: Error?) {
        ...
        facebookRequest.start(completionHandler: { _, result, error in
            guard let result = result as? [String: Any],
                error == nil else{
                print("Facebook graph req failed")
                return
            }
            
            guard let userName = result["name"] as? String,
                  let email = result["email"] as? String else {
                print("Failed get name n email from fb")
                return
            }
            ...
            FirebaseAuth.Auth.auth().signIn(with: credential, completion: { [weak self] authResult, error in
                ...
                guard authResult != nil, error == nil else {
                    if let error = error  {
                        print("Facebook credential failed \(error)")
                    }
                    return
                }
                print("Facebook login succeed")
                strongSelf.navigationController?.dismiss(animated: true, completion: nil)
            })
